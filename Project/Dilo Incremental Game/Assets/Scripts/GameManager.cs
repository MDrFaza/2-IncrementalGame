...
public class GameManager : MonoBehaviour
{
    ...
    { 
        ...
        if(IsRandomEvent == false && SpecialButton.active){
            SpecialButton.SetActive(false);
            StartCoroutine(WaitForEvent());
        }

        if(IsRandomEvent && SpecialButton.active == false){
            SpecialButton.SetActive(true);
            SpecialButton.GetComponent<RectTransform>().anchoredPosition = new Vector2(Random.Range(50,1200), Random.Range(50,500));
        } 
    }

    public void GetReward(){
        _totalGold = _totalGold + Random.Range(1, 500000);
        IsRandomEvent = false;
        StartCoroutine(WaitForEvent());
    }
